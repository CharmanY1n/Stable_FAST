...
from openpi.models import smolvlm_fast
...
class ModelTransformFactory(GroupFactory):
    ...
    def __call__(self, model_config: _model.BaseModelConfig) -> _transforms.Group:
        match model_config.model_type:
            ...
            case _model.ModelType.SMOLVLM_FAST:
                tokenizer = _tokenizer.SmolVLMTokenizer(
...
_CONFIGS = [
    ...
    TrainConfig(
        name="smolvlm_fast_libero",
        model=smolvlm_fast.SmolVLMFASTConfig(action_dim=7, action_horizon=10, max_token_len=180),
        data=LeRobotLiberoDataConfig(
            repo_id = "physical-intelligence/libero",
            base_config = DataConfig(prompt_from_task=True),
        ),
        weight_loader = weight_loaders.SmolVLM2WeightLoader(),
    ...
    TrainConfig(
        name="pi0_fast_libero_adaptive",
        model=pi0_fast.Pi0FASTConfig(action_dim=7, action_horizon=161, max_token_len=180,paligemma_variant="gemma_2b"),
        data=SimpleDataConfig(
            repo_id = "your_hf_username/processed_samples_last_action",
            assets = AssetsConfig(
                assets_dir = "your_asserts_dir",
                asset_id="norm_stats.json",
            ),
            data_transforms = lambda model: _transforms.Group(
                inputs = [
                    libero_policy.LiberoInputs(action_dim=model.action_dim, model_type=model.model_type),
            ...
            model_transforms = lambda model: _transforms.Group(
                inputs = [
                    _transforms.ResizeImages(224, 224),
                    _transforms.TokenizeFASTInputs(
                        _tokenizer.MyLocalFASTTokenizer(
                            max_len = model.max_token_len,
                            base_tokenizer_path = "paligemma_tokenizer.model",
                            fast_tokenizer_path = "adaptive_tokenizer",
                        )
                    ),
                ],
                outputs = [
                    _transforms.ExtractFASTActions(
                        tokenizer = _tokenizer.MyLocalFASTTokenizer(
                            max_len = model.max_token_len,
                            base_tokenizer_path = "paligemma_tokenizer.model",
                            fast_tokenizer_path = "adaptive_tokenizer",
                        ),
    ...
    TrainConfig(
        ...
        data=LeRobotLiberoDataConfig(
            repo_id="/home/yinxiaoran/data/datasets/libero_goal_no_noops",
            base_config=DataConfig(prompt_from_task=True),
        ),
