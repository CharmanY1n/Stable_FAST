...
class SmolVLMFASTConfig(_model.BaseModelConfig):
    dtype: str = "bfloat16"
    # SmolVLM-500M 使用 gemma_2b 变体
    ...
    def model_type(self) -> _model.ModelType:
        # 您可能需要在这里定义一个新的 ModelType
        return _model.ModelType.SMOLVLM_FAST

    @override
    def create(self, rng: at.KeyArrayLike) -> "SmolVLMFAST":
        return SmolVLMFAST(self, rngs=nnx.Rngs(rng))
    ...
    def get_freeze_filter(self) -> nnx.filterlib.Filter:
        ...
        if "lora" in self.paligemma_variant:
            return nnx.All(nnx.Any(nnx_utils.PathRegex(".*language_model.*"), nnx_utils.PathRegex(".*vision_tower.*")), nnx.Not(nnx_utils.PathRegex(".*lora.*")))
        return nnx.Nothing


class SmolVLMFAST(_model.BaseModel):
    def __init__(self, config: SmolVLMFASTConfig, rngs: nnx.Rngs):
    ...
    def embed_inputs(
        ...
        for name in obs.images:
            image_token_embeddings, _ = self.vision_tower(obs.images[name], train=False)
        ...
        tokenized_inputs_embeddings = self.language_model(obs.tokenized_prompt, embed_only=True)
    ...
    def compute_loss(
        ...
        targets = jax.nn.one_hot(
            ...
            self.language_model.module.vocab_size,
        )

        # Each input predicts *next* token, so we don't input the last token.
        pre_logits, _, _ = self.language_model(
    ...
    def sample_actions(
        ...
        *,
        ...
        prefix_logits, kv_cache, _ = self.language_model(
        ...
        def step(carry):
            ...
            # Decode one step
            token_embedding = self.language_model(token, embed_only=True)
            ...
            last_logit, kv_cache, _ = self.language_model(
